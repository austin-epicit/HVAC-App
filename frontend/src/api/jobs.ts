...
export const deleteJob = async (id: string): Promise<{ message: string; id: string }> => {
	const response = await api.delete<ApiResponse<{ message: string; id: string }>>(`/jobs/${id}`);
...
export const getJobVisitsByDateRange = async (startDate: Date, endDate: Date): Promise<JobVisit[]> => {
...
export const updateJobVisit = async (id: string, updates: UpdateJobVisitInput): Promise<JobVisit> => {
...
export const assignTechniciansToVisit = async (visitId: string, techIds: string[]): Promise<JobVisit> => {
	...
	if (!response.data.success) {
		throw new Error(response.data.error?.message || 'Failed to assign technicians to visit');
	}
...
export const updateJobNote = async (jobId: string, noteId: string, data: UpdateJobNoteInput): Promise<JobNote> => {
	const response = await api.put<ApiResponse<JobNote>>(`/jobs/${jobId}/notes/${noteId}`,data);
...
export const deleteJobNote = async (jobId: string, noteId: string): Promise<{ message: string }> => {
	const response = await api.delete<ApiResponse<{ message: string }>>(`/jobs/${jobId}/notes/${noteId}`);
